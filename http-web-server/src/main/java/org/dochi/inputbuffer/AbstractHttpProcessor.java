...
//// 2. AbstractHttpProcessor 구현체의 service(Request, Response, InternalAdapter) 호출
//// 3. InternalAdapter 객체가 HttpApiRequest 구현체와 HttpApiResponse 구현체에 각각 Request, Response 객체 주입 (각 AbstractProcessor 마다 InternalAdapter 객체를 생성해서 주입)
//// 4. HttpApiRequest 구현체에서 this.inputBuffer.setRequest(Request), InputBuffer로 멀티 파트등의 본문 파싱을 수행한 뒤에 Request에 해당 정보 저장 가능하기 위해서
//// 4. InternalAdapter 객체가 Request의 객체의 path를 확인해서 부합하는 HttpApiHandler 찾아낸다.
...
////    protected final InternalAdapter internalAdapter;
////
////
////    protected AbstractHttpProcessor(InternalAdapter internalAdapter) {
////        this.request = new Request();
////        this.response = new org.dochi.buffer.Http11ResponseProcessor();
...
////    }
//
//    protected final Http11RequestProcessor request;
//    protected final Http11ResponseProcessor response;
//
//    protected AbstractHttpProcessor(Http11RequestProcessor request, Http11ResponseProcessor response) {
...
//    }
//
//    protected abstract SocketState service(SocketWrapperBase<?> socketWrapper, HttpApiMapper httpApiMapper) throws SocketException;
//

...
import org.dochi.buffer.MimeHeaderField;
import org.dochi.buffer.SocketWrapperBase;
import org.dochi.internal.InputBuffer;
...
//        }
        ...
        // !this.buffer.hasRemaining(): limit >= position
        if (!this.buffer.hasRemaining() && this.socketInputBuffer.doRead(this) <= 0) {
            return -1;
        }
...
//    }
    ...
    // 1. 매개변수 타당성
    // ApplicationBufferHandler 로직 타당성 확인 필요

    // 2. 버퍼링
    // 바디 버퍼는 디폴트 8KB이다.
    // 버퍼링시, 7KB을 하고 6KB를 읽고 1KB가 빈 공간이다.
    // 즉, position: 6KB, limit: 7KB, capacity: 8KB
    // 따라서 입력시, capacity - limit == 1KB 만큼만 읽을수 있다.
    // 그러므로 읽은 데이터인 position만큼 왼쪽으로 쉬프트가 필요하다.
    // 그러면 limit - position 만큼의 데이터량을 0번 인덱스로 복사해야한다.
    // 이는 복사 비용이 발생한다.
    // 입력 작업에서 시스템 콜이 발생하게 되는데, 1KB만을 읽고 다시 버퍼를 초기화(position=0)해서 읽는다면 추가 시스템 콜 비용이 발생하게된다.
    // 아직 읽지 않은 데이터 만큼(limit - position)의 복사 비용과 시스템 콜 비용을 비교해야한다.


    // 매개변수를 통해 해당 메서드를 호출한 객체의 버퍼를 set 혹은 get 할수 있음
    @Override
    public int doRead(ApplicationBufferHandler handler) throws IOException {
        // position >= limit
        if (!this.buffer.hasRemaining()) {
            // 1. InputBuffer 클래스의 고유 버퍼가 this.buffer로 교체된 상태: 남은 용량 DEFAULT_BUFFER_SIZE 보다 작으면 expand
            // 2. InputBuffer 클래스의 고유 버퍼인 상태: 용량 DEFAULT_BUFFER_SIZE 보다 작으면 expand
            if (handler.getByteBuffer() == EMPTY_BUFFER) {
                handler.expand(DEFAULT_BUFFER_SIZE);
            } else if (this.buffer.array() == handler.getByteBuffer().array()) {
                handler.setByteBuffer(initBuffer(DEFAULT_BUFFER_SIZE));
            }

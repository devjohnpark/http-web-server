...
//    }
//
////    public static int parseValuesCrlfLine(InputBuffer inputBuffer, MessageBytes[] elements, int separator) throws IOException {
////        validateNullElements(elements);
////        int count = 0;
////        int previousByte = -1;
////        int currentByte;
////        int start = inputBuffer.getBufferPosition();
////        while ((currentByte = inputBuffer.getByte()) != -1) {
////            if (isSeparator(separator, currentByte) && !isExceedElementCount(count, elements.length)) {
////                elements[count++].setBytes(inputBuffer.getBuffer(), start, inputBuffer.getBufferPosition() - start - SEPARATOR_SIZE);
////                start = inputBuffer.getBufferPosition();
////            } else if (skipSpaceSuffixSeparator(separator, previousByte, currentByte)) {
////                start++;
////            } else if (isCRLF(previousByte, currentByte)) {
////                if (count <= 0) {
////                    return 0;
////                }
////                elements[count++].setBytes(inputBuffer.getBuffer(), start, inputBuffer.getBufferPosition() - start - CRLF_SIZE);
////                return count;
////            }
////            previousByte = currentByte;
////        }
...
//    // connector.Request에 RequestParameters 저장
//    // ? 기준으로 request-path와 query-string으로 파싱
//    // & 기준으로 parameters 나눈 후 = 기준으로 key와 value로 파싱
//}
